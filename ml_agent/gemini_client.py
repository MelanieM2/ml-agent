...
# Deliberately small at first, as a cheap debugging safety net, then
# raised once the loop was trusted end-to-end: many live runs across
# both datasets showed coherent, sensible tool sequencing with no sign
# of the runaway-loop failure mode the cap exists to catch, and a real
# run hit the previous, lower ceiling mid-retry -- cutting off
# legitimate in-progress reasoning rather than catching an actual bug.
MAX_ITERATIONS = 15
...
def run_agent_loop(
    ...
    *,
    ...
    is ever called. This module deliberately knows nothing about
    datasets, splits, or pos_label; its only job is talking to Gemini
    and dispatching whatever tool name comes back, keeping the "what
    dataset are we working on" logic and the "how do we talk to an LLM"
    logic in separate files.

    log_iterations: off by default. When True, every iteration is
    recorded -- tool name, tool arguments, result, and a UTC timestamp
    -- including the case where Gemini replies with plain text instead
    of a tool call (tool_name/tool_args/result are None on that kind of
    entry; response_text is populated instead). The full list is
    ...
    the log needs to persist across runs, that's the caller's
    responsibility, using result["log"].
    """
    client = genai.Client()
    ...
    # objects rather than passed as raw dicts. google-genai's
    # Tool(function_declarations=...) accepts raw dicts fine at runtime,
    # via Pydantic coercion -- but its own type stubs declare
    # list[FunctionDeclaration] | None, not dicts, so the explicit wrap
    # keeps static type-checking clean and doesn't rely on coercion
    # behavior the SDK doesn't actually commit to as part of its contract.
    ...
    # Uses the SDK's own chats.create() history tracking rather than
    # manually assembling a list of types.Content -- the simplest option
    # available, at the cost of no manual control over history pruning
    # (relevant once MAX_ITERATIONS grows large enough for request size
    # to matter). Worth revisiting deliberately if that need appears,
    # not switching silently.
    ...
    for iteration in range(max_iterations):
        ...
        if not function_calls:
            ...
            # but worth the caller knowing it happened this way rather than
            # via a proper record_convergence_decision call.
            if log_iterations:
                log.append({
                    "iteration": iteration,
                    "tool_name": None,
        ...
        # Still open: this assumes exactly one function_call per Gemini
        # turn. TOOL_SCHEMAS and dispatch_table are both built around
        # single calls, but Gemini's function-calling mode can in
        # principle return several parallel calls in one response --
        # that case isn't handled here.
        ...
        if tool_name == "record_model_proposal":
            # Still open: the human-in-the-loop extension point belongs
            # here, between record_model_proposal's return (result,
            # above) and train_model's call (which happens on Gemini's
            # *next* turn, driven by what it does with this fed-back
            # result). Currently auto-approves every proposal with no
            # human review. A real hook (e.g. an on_proposal callable
            # parameter) is separate, deferred work -- not implemented.
            pass
...
def format_log(log: list[dict[str, Any]]) -> str:
    ...
    block per iteration, separated for easy scanning in a terminal or a
    pasted-in doc. Purely a display helper; does not touch or
    reinterpret the log's actual data, so it's safe to call from
    anywhere that has a log list in hand. Used by main.py for its
    interactive CLI output; also usable from run_smoke_test.py, an
    uncommitted manual debugging script kept outside the real package.
    """
    blocks = []
    for entry in log:
